...
function create({tempDir, projectName, branch, file, slug, message = "Greenkeeper bitbucket commit", files = []}) {
    ...
    return execProcess(`git clone https://${this.auth.user}:${this.auth.pass}@bitbucket.org/${projectName}/${slug} ` +
        `&& cd ${slug} && git fetch origin ${branch} && git checkout ${branch} && mv ${file} . && git add package.json && git commit -m "${message}"  && git push origin ${branch} --force`, function(err,response) {

...
function create({tempDir, projectName, branch, file, slug, message = "Greenkeeper bitbucket commit"}) {
    if (!this.auth) {
    ...
    return new Promise((resolve, reject) => {
        execProcess(`git clone https://${this.auth.user}:${this.auth.pass}@bitbucket.org/${projectName}/${slug} ` +
            ...
            console.log(response);
                ...
                execProcess(`cd ${tempDir}/${slug} && git rev-parse HEAD`, function(err,response) {
